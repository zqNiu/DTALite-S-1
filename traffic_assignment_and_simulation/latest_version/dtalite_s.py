    ...
    for t in range(g_start_simu_interval_no,g_end_simu_interval_no,1):
        ...
        for no in range(0,len(g_node_list)):
			...
			#for each waited vehicle
                        ...
                        time_in_min=int(g_agent_list[agent_no].m_Veh_LinkArrivalTime_in_simu_interval[g_agent_list[agent_no].m_current_link_seq_no]/ g_number_of_interval_per_min)
...
def g_OutputFiles():
    with open ('link_performance.csv','w',newline='') as fp:
        ...
        for link in g_link_list:
            for abs_t in range(g_start_simu_interval_no,g_end_simu_interval_no,1):
                relative_t = g_A2R_simu_interval(abs_t)
                if(relative_t%(sampling_time_interval/g_number_of_seconds_per_interval)==0):
                    ...
                    line=[link.link_seq_no,link.extern_from_node,link.extern_to_node,time_stamp_to_HHMMSS(time_in_min)+"_"+time_stamp_to_HHMMSS(time_in_min+1),
